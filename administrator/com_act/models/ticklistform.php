 ...
 * @since  1.6
 */
class ActModelTicklistForm extends JModelForm
{
    private $item = null;
    ...
    protected function populateState()
    {
        ...
        {
                $id = Factory::getApplication()->getUserState('com_act.edit.ticklist.id');
        }
        else
        {
                $id = Factory::getApplication()->input->get('id');
                Factory::getApplication()->setUserState('com_act.edit.ticklist.id', $id);
        }

        $this->setState('ticklist.id', $id);
        ...
        {
                $this->setState('ticklist.id', $params_array['item_id']);
        }
     ...
     */
    public function getTable($type = 'Ticklist', $prefix = 'ActTable', $config = array())
    {
    ...
    {
        // Get the id.
        $id = (!empty($id)) ? $id : (int) $this->getState('ticklist.id');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_act.ticklist', 'ticklistform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_act.edit.ticklist.data', array());
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ticklist.id');
    ...
    public function delete($pk)
    {
        $user = Factory::getUser();
            ...
            if (empty($pk))
            {
                    $pk = (int) $this->getState('ticklist.id');
            }
