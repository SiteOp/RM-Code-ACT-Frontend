...
// USE FILTER 
$use_filter_c_grade     = $params['use_filter_c_grade']; // Filter C-Grade
$use_filter_vr_grade     = $params['use_filter_vr_grade']; // Filter C-Grade
...
$style .=  0==$use_filter_c_grade  ? '#filter_cgrade, #filter_cgrade_chosen {display: none;}' : ''; // Filter ausblenden
$style .=  0==$use_filter_vr_grade ? '#filter_vrgrade, #filter_vrgrade_chosen {display: none;}' : ''; // Filter ausblenden
    ...
    <div class="table-responsive ">
        <table class="table table-striped table-sm" id="routeList">
            ...
            <tr>
                ...
                <th class="r_admininfo d-none d-xl-table-cell" ><?php // Route Info ? Admin ?>
                    <?php echo HTMLHelper::_('grid.sort', 'COM_ACT_ROUTE_ADMIN_INFO', 'a.infoadmin', $listDirn, $listOrder); ?>
                </th>
            ...
            <?php foreach ($this->items as $i => $item) : ?>
                <tr class="row<?php echo $i % 2; ?>">
                    ...
                    <td class=""> 
                        ...
                        <?php // Linienoptionen z. B. Auto Belay ?>
                        <?php $options = explode(",", $item->lineoption); ?>
                        <?php foreach ($options as $option) : ?>
                            <?php echo ActHelpersAct::getLineoptions($option, 0); ?>
                        <?php endforeach ; ?>
                    </td>
                    ...
                    <td class="d-none d-xl-table-cell"><?php // Info Route Admin? ?>
                        <?php if (!empty($item->infoadmin)) : ?>
                            <a rel="popover" 
                                data-placement="bottom" 
                                data-trigger="hover" 
                                data-content="<?php echo $item->infoadmin; ?>" 
                                >
                                    <?php echo HTMLHelper::_('string.truncate', $item->infoadmin, 12, false, false ); ?>
                            </a>
                        <?php endif; ?>
                    </td>
