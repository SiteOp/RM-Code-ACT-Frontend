...
$use_routesetter = $params->get('use_routesetter', 0);

// User permissions
$user = Factory::getUser();
$listOrder = $this->state->get('list.ordering');
$listDirn = $this->state->get('list.direction');
$canCreate = $user->authorise('core.create', 'com_act') && file_exists(JPATH_COMPONENT . '/models/forms/commentform.xml');
$canEdit = $user->authorise('core.edit', 'com_act') && file_exists(JPATH_COMPONENT . '/models/forms/commentform.xml');
...
// Helper Colors
JLoader::import('helpers.colors', JPATH_SITE . '/components/com_act');
?>
...
<form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="get" name="adminForm" id="adminForm">
    ...
    <?php // Statistik Kommentare Charts.js ?>
    <div class="row mb-5 d-none d-md-block">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <canvas id="commentsChart" style="position: relative; height: 27vh;"></canvas>
                </div>
            </div>
        </div>
    </div>
            ...
            <tbody>
                <?php foreach ($this->items as $i => $item) : ?>
                    ...
                    <tr class="row<?php echo $i % 2; ?>">
                            ...
                            <a rel="popover" 
                                ...
                                data-content="<div class='pop_line'>Linie: <?php echo $item->line; ?></div>
                                              ...
                                              <div class='pop_cgrade'>C-Grade:
                                                  <?php if ($item->c_grade == 0) : ?>
                                                    <?php echo '-'; ?>
                                                  <?php else : ?>
                                                    <?php echo $item->c_grade; ?>
                                                  <?php endif; ?>
                                               </div>">
                                <i class="<?php echo Text::_('COM_ACT_FA_INFO'); ?> pr-2 <?php echo ($item->route_state == 1 || $item->route_state == -1) ? 'route_active' : 'route_archiv'; ?>"></i>
                            </a>
                            <?php if (($item->route_state == 1) || ($item->route_state == -1) || ($adminCommentEdit)) : ?>
                                <a href="<?php echo Route::_('index.php?option=com_act&view=route&id=' . (int) $item->route_id); ?>">
                                    <?php echo $this->escape($item->route_name); ?>
                                </a>
                            <?php else : ?>
                                <?php echo $this->escape($item->route_name); ?>
                        ...
                        <td>
                            <?php if (ActHelpersAct::getUserProfilAscentShow($item->created_by) && $item->ticklist_yn == 1) : ?>
                                <?php echo ActHelpersAct::getUserAscentIcon($item->ascent); ?>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if ($adminCommentEdit) : ?>
                                <a href="<?php echo Route::_('index.php?option=com_act&view=comments'); ?>?filter[user]=<?php echo Factory::getUser($item->created_by)->username; ?>">
                        ...
                        <?php if ($adminCommentEdit) : ?>
                            <td class="text-center">
                                <?php 
                                $inputIcons = [
                                    1 => ['class' => 'fas fa-laptop', 'color' => '#0393ad', 'title' => 'Website'],
                                    2 => ['class' => 'fab fa-android fa-lg', 'color' => '#78C257', 'title' => 'Android'],
                                    3 => ['class' => 'fab fa-apple fa-lg', 'color' => '#777', 'title' => 'iOS']
                                ];
                                
                                $icon = $inputIcons[$item->input] ?? ['class' => 'fas fa-question', 'color' => '#fab903', 'title' => 'Unknown'];
                                ?>
                                <a class="" rel="popover" 
                                   data-container="body"
                                   data-placement="top" 
                                   data-trigger="hover" 
                                   data-content="<?php echo $icon['title']; ?>">
