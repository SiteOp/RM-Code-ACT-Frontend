 ...
 * @since  1.6
 */
class ActModelCommentForm extends JModelForm
{
    private $item = null;
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_act.comment', 'commentform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_act.edit.comment.data', array());
    ...
    public function delete($pk)
    {
        $user = Factory::getUser();
            ...
            if (empty($pk))
            {
                    $pk = (int) $this->getState('comment.id');
            }
    ...
    /**
     * Method to get the name of the route
     *
     * @param   int $type   id from the comment

     * @return  string Name from route
     */
	...
	{ 
		...
		$query->select(array( 'r.name AS routename', 'r.settergrade', 'r.state'))
			  ->from('#__act_route AS r')
              ->join('LEFT', '#__act_comment AS c ON c.route = r.id')
