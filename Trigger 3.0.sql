...
BEGIN
    ...
    SELECT (CASE WHEN t.sum_cgrade = 0 THEN r.settergrade ELSE (round((((r.settergrade * 1.99)  + t.sum_cgrade) / (1.99 +  t.count_cgrade)),3)) END)
...
BEGIN
    ...
    SELECT (CASE WHEN t.sum_cgrade = 0 THEN r.settergrade ELSE (round((((r.settergrade * 1.99)  + t.sum_cgrade) / (1.99 +  t.count_cgrade)),0)) END)
    FROM g9p5k_act_trigger_calc as t
    RIGHT OUTER JOIN g9p5k_act_route AS r on r.id = t.id
    WHERE t.id = new.route 
    INTO calc_grade;
